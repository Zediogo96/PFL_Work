...
superior(X,Y) :- cargo(CargoX, X), cargo(CargoY, Y), chefiado_por(CargoY, CargoX).
superior(X,Y) :- cargo(CargoX, X), cargo(CargoY, Y), chefiado_por(CargoY, C2), cargo(C2, P), superior(X,P).